...
// ── Welcome Screen (first visit - ask name) ──
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, "users", uid), {
        uid, name: name.trim(),
        joinDate, lastActive: joinDate,
        timestamp: serverTimestamp(),
      });
      localStorage.setItem("nt_name", name.trim());
      localStorage.setItem("nt_joined", joinDate);
  ...
  return (
    <div style={{position:"fixed",inset:0,width:"100vw",height:"100vh",background:`linear-gradient(135deg, #1A2B4A 0%, #2B4B8C 50%, #1A2B4A 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:"20px",fontFamily:"'Inter',sans-serif",overflow:"auto",boxSizing:"border-box"}}>
      <div style={{background:"white",borderRadius:24,padding:40,width:"100%",maxWidth:420,boxShadow:"0 32px 80px rgba(0,0,0,0.3)",textAlign:"center",boxSizing:"border-box",marginLeft:"auto",marginRight:"auto"}}>
        ...
        <h1 style={{fontSize:26,fontWeight:800,color:"#1A2B4A",margin:"0 0 6px"}}>Welcome to NutriTrack</h1>
        <p style={{color:"#6B8299",fontSize:14,marginBottom:28,lineHeight:1.6}}>Track your nutrition, reach your goals.<br/>Let's start with your name!</p>

        <div style={{textAlign:"left",marginBottom:16}}>
          <label style={{display:"block",fontSize:11,color:"#6B8299",marginBottom:6,textTransform:"uppercase",letterSpacing:"0.07em",fontWeight:600}}>Your Name</label>
          <input
            type="text"
            ...
            autoFocus
            style={{background:"#EEF4FB",border:"1.5px solid #D5E3F0",borderRadius:12,color:"#1A2B4A",padding:"14px 16px",fontSize:15,fontFamily:"'Inter',sans-serif",outline:"none",width:"100%",boxSizing:"border-box",fontWeight:500}}
          />
        </div>
        ...
        <button
          ...
          style={{width:"100%",background:"linear-gradient(135deg,#1A2B4A,#2B6CB0)",border:"none",color:"white",padding:"14px",borderRadius:12,fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:16,cursor:name.trim()?"pointer":"not-allowed",opacity:name.trim()?1:0.5,boxShadow:"0 6px 20px rgba(26,43,74,0.3)",transition:"all 0.2s"}}>
          {loading ? "Setting up..." : "Start Tracking 🚀"}
        </button>
        <p style={{color:"#9BB0C4",fontSize:11,marginTop:14}}>No account needed · Free forever · 150+ Indian foods</p>
      </div>
    </div>
  );
}
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.card,borderRadius:20,width:"100%",maxWidth:380,padding:28,boxShadow:"0 24px 60px rgba(26,43,74,0.25)",border:`1px solid ${C.border}`}}>
        ...
        <div style={{position:"relative",marginBottom:8}}>
          <input
            ...
            autoFocus
            style={{background:C.bg,border:`1.5px solid ${error?"#C53030":C.border}`,borderRadius:10,color:C.text,padding:"12px 44px 12px 14px",fontSize:14,fontFamily:"'Inter',sans-serif",outline:"none",width:"100%",boxSizing:"border-box",transition:"border 0.2s"}}
          />
          <button onClick={()=>setShow(p=>!p)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.muted,fontSize:16,padding:4}}>
            {show?"🙈":"👁️"}
          </button>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:12}}>
          ...
          <button onClick={tryLogin} style={{flex:2,background:"linear-gradient(135deg,#1A2B4A,#2B6CB0)",border:"none",color:"white",padding:11,borderRadius:10,cursor:"pointer",fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:14}}>
            🔐 Unlock Admin
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  const [showWelcome, setShowWelcome] = useState(!savedName);
  ...
  const [prof, setProf]       = useState(()=>{ try{return JSON.parse(localStorage.getItem("nt_prof")||JSON.stringify({name:savedName||"User",weight:70,height:170,age:25,gender:"male",activity:"moderate"}))}catch{return{name:savedName||"User",weight:70,height:170,age:25,gender:"male",activity:"moderate"}} });
  ...
  // Track visit + update lastActive
  useEffect(()=>{
    if(showWelcome) return;
    const uid = getUserId();
    const trackVisit = async () => {
      try {
        await addDoc(collection(db,"visits"),{ userId:uid, userName:localStorage.getItem("nt_name")||"Unknown", date:today, page:"app", timestamp:serverTimestamp() });
        await setDoc(doc(db,"users",uid),{ lastActive:today },{ merge:true });
      } catch(e){ console.log("Track:",e); }
    };
    trackVisit();
  },[showWelcome]);

  const handleWelcomeDone = (name) => {
    setUserName(name);
    setProf(p=>({...p,name}));
    ...
    <div style={{width:"100vw",minHeight:"100vh",background:C.bg,fontFamily:"'Inter',sans-serif",color:C.text,overflowX:"hidden"}}>
      ...
      {!isMobile&&(
        <div style={{display:"flex",width:"100%",minHeight:"100vh"}}>
          <aside style={{width:sidebarW,minHeight:"100vh",background:C.sidebar,display:"flex",flexDirection:"column",position:"fixed",left:0,top:0,bottom:0,zIndex:200}}>
            <div style={{padding:"24px 18px 20px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div onClick={handleLogoClick} style={{width:42,height:42,borderRadius:12,background:"linear-gradient(135deg,#3A7BD5,#2B6CB0)",display:"flex",alignItems:"center",justifyContent:"center",color:"white",flexShrink:0,boxShadow:"0 4px 14px rgba(0,0,0,0.3)",cursor:"pointer",userSelect:"none"}}>{Icons.leaf}</div>
                <div><div style={{fontWeight:700,fontSize:17,color:"white"}}>NutriTrack</div><div style={{fontSize:10,color:"rgba(255,255,255,0.45)"}}>Fuel your goals</div></div>
              </div>
            </div>
            ...
            <nav style={{padding:"12px 10px",flex:1}}>
              {navItems.map(n=>(
                <button key={n.id} onClick={()=>setTab(n.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"11px 12px",borderRadius:11,border:"none",background:tab===n.id?"rgba(255,255,255,0.12)":"transparent",color:tab===n.id?"white":"rgba(255,255,255,0.45)",cursor:"pointer",fontFamily:"'Inter',sans-serif",fontSize:14,fontWeight:tab===n.id?700:400,marginBottom:3,transition:"all 0.2s",textAlign:"left"}}>
                  {n.icon}{n.label}{tab===n.id&&<div style={{marginLeft:"auto",width:4,height:18,borderRadius:99,background:"#3A7BD5",flexShrink:0}}/>}
                </button>
              ))}
            </nav>
            <div style={{padding:"14px 16px",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:36,height:36,borderRadius:"50%",background:"linear-gradient(135deg,#3A7BD5,#2B6CB0)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:16,color:"white",flexShrink:0}}>{(userName||prof.name)[0]?.toUpperCase()}</div>
                <div style={{overflow:"hidden"}}>
                  <div style={{fontSize:13,fontWeight:600,color:"white",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{userName||prof.name}</div>
      ...
      {isMobile&&(
        <>
          ...
          <div style={{padding:"16px 14px 88px"}}><div className="page-anim" key={tab}>{page()}</div></div>
          <nav style={{position:"fixed",bottom:0,left:0,right:0,background:C.sidebar,display:"flex",zIndex:200,boxShadow:"0 -4px 20px rgba(26,43,74,0.2)"}}>
            {navItems.map(n=>(
